(() => {
    ...
    onState(data, ctx) {
      ...
      if (phase === 'spin') {
        ...
        document.getElementById('current-hint').textContent = isHost
          ? '点击按钮转动瓶子，随机选出本轮玩家'
          : '等待房主转动瓶子…';
        PartyRoom.setHostActions(['spin-btn', 'back-setup-btn'], isHost);
      ...
      if (phase === 'choose') {
        ...
        PartyRoom.setHostActions(['back-setup-btn-choose'], isHost);
  ...
  function bindRoomLevelPicker() {
    const group = document.getElementById('room-create-level-group');
    if (!group) return;
  ...
  function bindLobbyLevelPicker() {
    const group = document.getElementById('room-lobby-level-group');
    if (!group) return;
    group.addEventListener('click', (event) => {
      const btn = event.target.closest('.game-level-btn');
      if (!btn || btn.disabled || !room.state.roomId || !room.state.isHost) return;
  ...
  function bindRoomGameActions() {
    ...
    document.getElementById('swap-btn').addEventListener('click', (event) => {
      if (!room.state.roomId) return;
      event.stopImmediatePropagation();
      room.gameAction('room_swap').catch((err) => window.alert(err.message));
    }, true);

    ['back-setup-btn', 'back-setup-btn-choose'].forEach((id) => {
      const el = document.getElementById(id);
      if (!el) return;
      el.addEventListener('click', (event) => {
        if (!room.state.roomId) return;
        event.stopImmediatePropagation();
        room.gameAction('room_back').catch((err) => window.alert(err.message));
      }, true);
    });
  }

  bindRoomLevelPicker();
  bindLobbyLevelPicker();
  bindRoomGameActions();
  room.tryResume();
})();
